...
export default function CustomerStatement() {
  ...
  const generateBankStatement = async () => {
    ...
    const summaryData = [
      ['Total Bills:', stats.totalBills.toString(), 'Total Purchases:', `Rs. ${Math.round(stats.totalPurchases).toLocaleString()}`],
      ['Paid Bills:', stats.paidBills.toString(), 'Total Paid:', `Rs. ${Math.round(stats.totalPaid).toLocaleString()}`],
      ['Unpaid Bills:', stats.unpaidBills.toString(), 'Outstanding:', `Rs. ${Math.round(stats.totalOutstanding).toLocaleString()}`],
    ];
  ...
  const generateStatementPDFBlob = (): Blob | null => {
    ...
    pdf.setFontSize(11);
    pdf.text(`Rs. ${Math.round(stats.totalPurchases).toLocaleString()}`, margin + colWidth / 2, yPos, { align: 'center' });
    pdf.text(`Rs. ${Math.round(stats.totalPaid).toLocaleString()}`, margin + colWidth + colWidth / 2, yPos, { align: 'center' });
    pdf.text(`Rs. ${Math.round(stats.totalOutstanding).toLocaleString()}`, margin + 2 * colWidth + colWidth / 2, yPos, { align: 'center' });
    yPos += 15;
    ...
    pdf.text(`Rs. ${Math.round(stats.totalOutstanding).toLocaleString()}`, pageWidth - margin - 5, yPos + 3, { align: 'right' });
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
      try {
        await navigator.share({
          ...
          text: `Account Statement from ${receiptSettings.businessName} - Balance: Rs. ${Math.round(stats.totalOutstanding).toLocaleString()}`
        });
        toast({
...
Total Purchases: Rs. ${Math.round(stats.totalPurchases).toLocaleString()}
Total Paid: Rs. ${Math.round(stats.totalPaid).toLocaleString()}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="stat-card p-4 rounded-xl text-center">
                ...
                <p className="text-xl font-bold text-slate-800" data-testid="text-total-purchases">
                  Rs. {Math.round(stats.totalPurchases).toLocaleString()}
                </p>
                ...
                {unpaidSales.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {unpaidSales.map((sale) => {
                      ...
                      const paidPercent = (parseFloat(sale.amountPaid) / parseFloat(sale.totalAmount)) * 100;
                      return (
                        <div
                          key={sale.id}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4">
                            <div className="text-right">
                              <p className="text-sm text-slate-500">
                                Rs. {Math.round(parseFloat(sale.amountPaid)).toLocaleString()} / Rs. {Math.round(parseFloat(sale.totalAmount)).toLocaleString()}
                              </p>
      ...
      <Dialog open={deletePaymentDialogOpen} onOpenChange={setDeletePaymentDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this payment of Rs. {paymentToDelete ? Math.round(parseFloat(paymentToDelete.amount)).toLocaleString() : 0}? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
