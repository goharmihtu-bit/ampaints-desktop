...
- **Unpaid Bills & Customer Statements**: Tracks partial payments, due dates, manual balance additions (Manual Balance), and generates detailed PDF statements. Features a premium bank-style customer statement with transaction ledger, running balance, and various transaction type displays. Transaction History now correctly shows initial payments (at POS) as CREDIT entries alongside recovery payments, ensuring PAID bills show balanced debits and credits with a 0 balance.
...
- **Reporting**: Comprehensive financial reports with summary cards (Total Sales, Paid, Outstanding, Recovery Payments, Total Returns), detailed views (All Sales, Unpaid Bills, Recovery Payments, Returns), advanced filtering, and sortable columns. Features a unified Transactions tab that shows all bills (debits), payments (credits), and returns (credits) in a single timeline with clear type labels and debit/credit accounting. All outstanding balance calculations properly account for return credits.
  - **Store Cash Balance**: Prominent card in Overview tab showing actual cash received for the selected date range. Uses TRUE CASH FLOW calculation that properly attributes cash to when it was received:
    - `Sales (at POS)` = Initial payments on new sales (amountPaid - recovery payments for those sales)
    - `Recovery Payments` = All payment_history entries in date range (for any sale, old or new)
    - `Net Cash` = Sales + Recovery - Refunds
    - This avoids double-counting and properly handles historical date ranges.
